...
<section id="main-content">
    <div class="row mt">
        <div class="container">
            <h3 class="page-header" style="">Today's Offers</h3>
    ...
    <form id="addvendorform" method="POST" enctype="multipart/form-data">    
        <button type="button" id="vendor-btn" style="background-color: #68DFF0;padding: 10px 35px;">Add Today Offers</button>
        <br/>  
        <div id="output"></div>  
        ...
        <div id="dialog" class="web_dialog">  
            <div class="pop_main" id="add_vendor">
                ...
                <div id="id_Title" class="pop_header">Add Today Offer's</div>
                <div id="resp_msg"></div>
                <div class="pop_container" >
                    ...
                    <div class="right" id="grouppopupoperation" style="float:right;padding:0px 103px 0px 0px;">
                        <input type="button" id="btnCancel" class="btn logout" style="padding: 0px 20px 2px 20px;"  value="Cancel"/>
                        <input type="button" id="add_button" class="btn btn-default" style="padding: 0px 20px 2px 20px;" value="Add"/>
                    </div>
                </div>
            </div>
        </div>  
    ...
    <br style="clear:both;"/>
    <div id="todaysOffGridDiv">
        <div id="todaysOffGrid" class="easyui-datagrid" rownumbers="true" pagination="true" style="width: auto">
        </div>
    </div>
...
<script>
    ...
    function initDataGrid() {
        ...
        $('#todaysOffGrid').datagrid({
            url: 'todaysOffGrid?' + data,
            columns: [[
//                            {field: 'id', checkbox: true},
            ...
            onBeforeLoad: function () {
                $('#todaysOffGrid').find('.pagination').hide();
            }
            //                   fit:true
        });
        $('#todaysOffGrid').datagrid('getPager').pagination({
            layout: ['list', 'first', 'prev', 'sep', 'manual', 'sep', 'next', 'last', 'sep', 'refresh'],
            displayMsg: 'Displaying {from} to {to} of {total} Groups'
        });
        $("#todaysOffGrid").on("click", function () {
            $("#divPopStatus").html(' ');
            $("#statusDiv").html(' ');
        });
    }
    function checkRecordsInGrid() {
        $('#todaysOffGrid').find('.pagination').show();
        if ($('#todaysOffGrid').datagrid('getRows').length == "0") {
    ...
    function updateOffer(rowIndex) {
        $("#image_preview").html("");
        $("#output").html('');
        var selectedRow = $('#todaysOffGrid').datagrid('getRows')[rowIndex];
